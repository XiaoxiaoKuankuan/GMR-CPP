...
完整音乐离线重定向使用同一个 frame-shared 接触 QP 同时处理人体跟随、根/腿时序、
鞋底对地和防穿地。当前鞋底平面只做轻微纠偏：1.5° 内为 tilt 死区，超过后使用较低
增益缓慢拉回，前后/左右硬走廊为 6 mm，过渡持续 12 帧。QP 内另有权重 0.25 的
零-jerk 软目标，优先保持相邻帧加速度连续，但防穿地硬约束仍拥有更高优先级。

``frame_jerk_weight`` 不是输出低通滤波；它不会在求解后改写 qpos，也不会等待未来帧。
它仅在启用 frame temporal limits 的 batch 路径生效。实时服务共享 tilt 死区和低增益，
仍由实时 motion guard 限制速度、加速度和手臂换解。

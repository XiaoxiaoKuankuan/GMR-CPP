...
## 2026-08-27：batch 帧共享时序 QP，消除跨 IK 分支跳变

### 问题与根因

上一版已经修正落地和悬浮，但 `solveIK()` 对两张匹配表各自最多执行 11 次内部迭代，
每次迭代都按 MuJoCo 内部 `0.005 s` 重新发放速度额度。因此 30 Hz 的一个 SMP1 输入帧
可以累计花掉十几份额度：FineDance `finedance_01` 在第 612/965 帧出现
`58.31°/50.40°` 的手臂换解，第 1012/1662 帧又出现 `3.76/3.09 cm` 的根节点 Z 修正。
batch 同时没有构造实时服务已有的 `RealtimeMotionGuard`，最终 `projectContactConfiguration()`
还能绕过 QP 直接修改根节点 Z。

### 主要实现

- 新增“每个输入帧一份总额度”：帧开始时根据真实 SMP1 `dt`、上一输出速度和
  速度/加速度上限生成累计位移区间；两张 IK 表及其全部内部迭代都只能消费这一个区间，
  不再按迭代重复领额度；
- 双腿、腰、21 个单自由度关节、free-root 的 XYZ 和旋转与脚底不穿地、支撑高度、
  鞋底平面及 XY 防滑一起进入 DAQP。帧共享模式不再执行 QP 后的根节点 Z 直接投影，
  也删除旧 batch 的输出后 XY 插值；
- 根节点水平速度 `0.75 m/s` 与角速度 `6 rad/s` 现在按向量总量保守分配到各轴，
  Z 速度为 `0.45 m/s`，线/角加速度为 `3/20`；batch 接触 QP 的关节跟踪上限从
  执行器极限 `12 rad/s` 收紧为 `6 rad/s`，理论 30 Hz 单帧上限由 `22.92°` 降为
  `11.46°`。实时服务仍保留 Noetix 的 `9/12 rad/s` 执行器安全表；
- batch 热身和初始接触收敛完成后才启用共享时序 QP 与两帧手臂分支确认。QP 后保护器
  只处理手臂，腿和腰不再被二次改写而破坏刚求出的脚底可行性；
- 接触过渡只在上一帧 QP 成功后收紧，首次捕获的过渡带必须先实际求解一次；失败时
  保持当前带宽。速度区间始终允许急刹到零，但不允许一帧反向跳变，避免接触切换连续
  冻结多帧。

### 实际验证

- `smplx_bumi3_config_test` 的突变回归确认 27 个自由度全部受同一 30 Hz 帧预算约束，
  根节点首帧线速度 `0.057735 m/s`、关节最大 `2.66667 rad/s`，双脚最低点
  `1.61/1.59 mm`，测试通过；
- 完整 FineDance `finedance_01` 共 1851 帧：关节单帧最大值从 `58.31°` 降到
  `11.46°`，`>30°` 从 9 帧降到 0；关节加速度 P95/最大从
  `230.88/938.75` 降到 `81.64/173.37 rad/s²`；
- 根节点单帧最大位移从 `3.841 cm` 降到 `2.072 cm`，Z 最大从 `3.760 cm`
  降到 `1.500 cm`，Z `>2 cm` 从 11 帧降到 0；原第 1012/1662 帧 Z 变化降为
  `0.038/0.192 cm`；
- 全段没有完全重复的冻结帧，最终回放没有接触 QP 失败；定义的左右鞋底点最低值
  `5.30e-7 m`，无穿地。正式对照视频为 1851 帧、30 FPS、61.7 秒且完整音频可解码。

### 兼容边界

新 batch binary 与 IK JSON SHA 均已改变，旧 qpos 必须重新生成。`6 rad/s` 是离线
接触重定向为平滑性主动收紧的跟踪上限，不改写机器人执行器的 `9/12 rad/s` 物理来源。
本次证明的是运动学时序连续、脚底几何不穿地和媒体完整性；GMT 动力学稳定仍需后续
控制仿真或实机验证。
...
- 短时无 Redis 启动检查确认默认打印 `ground_clearance=0.040`、脚接触开启和实时
  安全开启。
# 2026-08-27：batch 支撑脚平面约束避免长时间整机停帧

- 60 首完整音乐回归发现，强落脚时脚跟到脚尖可能仍相差 5--10 cm；旧实现要求硬平面
  过渡带每帧必须收紧固定的六分之一，但腿和根节点同时受整帧速度/加速度预算限制，二者
  冲突时 DAQP 会连续失败，安全回退只能保持整机不动，最长形成数秒可见停顿。
- 平面约束现在采用“只许变好、不许变坏”的可行过渡带：落脚倾斜较大时允许在当前角度
  刹住或逐帧变平，软法向目标持续使用当帧剩余预算；进入 2--3 mm 目标带后仍恢复严格
  鞋底平面范围。
- 极少数双脚同时支撑且线性化仍冲突时，只临时放松平面行，鞋底网格不穿地仍是不可放松
  的硬约束。这样优先少跟人体动作，而不是让全身连续停帧或跳到另一套 IK 解。
- 完整 60 首的鞋底点回归还发现：只保护“求解前最低的 8 个网格点”时，踝关节在单帧
  限速范围内转动后可能换成另一组脚尖/脚跟点最低。现在四个配置鞋底角点也始终加入硬
  防穿地约束，并用 ``sole_corner_floor_margin_m=0.0027`` 预留线性化和网格最低点差异，
  防止大倾角落脚后的新最低点穿过地面。
- 当前最低的网格保护点另保留 ``mesh_floor_margin_m=0.0006``（0.6 mm）数值余量，
  避免 DAQP/MuJoCo 线性化与浮点误差把理论上的 0 高度写成几十微米的负数。

## 2026-08-27：接触 QP 轻量 jerk 抑制与鞋底倾角死区

### 修改目标

上一版已经消除多秒停帧和大幅跳解，但 3 mm 强平面走廊配合较高的
``tilt_weight=60``、``tilt_kp=8``，会在人体脚姿态和接触活跃集小幅变化时持续驱动
踝、膝来回修正。本次明确把“轨迹不抖”置于“鞋底每帧绝对水平”之前，只保留轻微的
鞋底平面纠偏，同时不放松实际鞋 mesh 防穿地。

### 主要实现

- ``gmr_mink.hpp`` 在 batch 已启用的整帧时序/接触 QP 中加入 jerk 软目标。它根据
  上一帧速度和加速度预测零 jerk 的本帧位移，并在每次内部 IK 迭代中对“剩余位移”
  加同一个二次目标；目标与人体跟随、根/腿限制和脚接触由 DAQP 同时求解，不在 QP
  之后滤波或改写 qpos。硬接触需要时可以压过该软目标。
- 鞋底法向目标增加 ``tilt_deadzone_rad``。倾角在死区内时完全不修 roll/pitch；超过
  死区后只修正超出的部分，yaw 始终不受鞋底目标约束。
- ``foot_contact.constraints`` 最终采用 ``frame_jerk_weight=0.25``、
  ``tilt_weight=12``、``tilt_kp=2.5``、``tilt_deadzone_rad=0.02618``（1.5°）；
  前后/左右硬走廊由 3 mm 放宽到 6 mm，接触过渡由 6 帧延长为 12 帧。
- 鞋底中心 1--3 mm、0.6 mm mesh 数值余量、2.7 mm 鞋底角扫掠余量和不可放松的
  防穿地行保持不变。新增配置字段默认分别为 0，旧配置不写这两个字段时保持兼容。

### 动态验证

- 固定 ``aistpp_01``、``finedance_01``、``finedance_20``、``compas3d_03`` 共
  17,662 帧：与上一版强平面输出相比，12 个腿关节 P95 加速度由 58.22 降到
  49.97 rad/s²，9 帧高频残差 RMS 由 0.730° 降到 0.642°，P95 jerk 由
  2078.71 降到 1753.98 rad/s³；没有大于 12° 的单帧关节步。
- 与同输入“近似关闭平面目标”的基线相比，腿部 P95 加速度为 49.97 对
  49.56 rad/s²，高频残差为 0.642° 对 0.633°，已接近原先不强制鞋底水平时的平滑度。
- 鞋底中心不高于 5 mm 的样本中，前后/左右高度差 P95 为 2.53/3.01 mm，95.53%
  同时不超过 3.5 mm，全部不超过新的 6 mm 安全走廊。逐帧检查实际鞋 mesh 最低值为
  ``-1.38e-8 m``，低于 ``-1 µm`` 的帧为 0。

### 功能边界

jerk 目标依赖 ``enableFrameTemporalLimits()``，因此当前作用于 batch server 的
整帧接触 QP；实时 server 仍使用因果 ``RealtimeMotionGuard``，但会共享更低的鞋底
tilt 增益和死区。上述数值证明离线运动学轨迹更平滑且不穿地，不替代 GMT 闭环动力学
或实机稳定性验证。

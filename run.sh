#!/bin/bash
# run.sh — Launch GMR-CPP optitrack mocap server
set -e
...
#IK_CONFIG="$SCRIPT_DIR/config/ik_configs/fbx_to_g1.json"
IK_CONFIG="$SCRIPT_DIR/config/ik_configs/calibrated/fbx_to_g1.json"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Optional:"
            echo "  --xml <path>        MuJoCo XML (default: assets/unitree_g1/g1_mocap_29dof.xml)"
            echo "  --ik-config <path>  IK config JSON (default: config/ik_configs/fbx_to_g1.json)"
            echo "  --redis-host <ip>   Redis host (default: 127.0.0.1)"
            echo "  --redis-port <port> Redis port (default: 6379)"
            echo "  --redis-key <key>   Redis key (default: mmocap_motion_frame_g1)"
...
# ── Print config ───────────────────────────────────────────────────────────────
echo "=============================================="
echo "  GMR-CPP OptiTrack Mocap Server"
echo "=============================================="
